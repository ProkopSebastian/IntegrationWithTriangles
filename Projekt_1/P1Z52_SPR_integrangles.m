function [res] = P1Z52_SPR_integrangles(f, n)
...
P = 2/(4*n^2); % Pole trójkąta
d2 = 1/n; % Długość przekątnej małego kwadratu
vA_x = linspace(-1, 0-d2, n);
vA_y = linspace(0, 1-d2, n);
% Wszystkie potrzebne punkty w każdym zestawie czterech trójkątów jakim
% jest kwadrat można znaleźć znając poniższe współrzędne. A,B,C,D
% odpowiadają wierzchołkom kwadratu jaki stanowią 4 trójkąty.
vA_x = vA_x';
vA_y = vA_y';
vB_x = vA_x + d2;
vB_y = vA_y + d2;
...
% Wzór wewnątrz pętli to suma wzorów dla 4 trójkątów stanowiących kwadrat
% Obliczane są jednocześnie wartości n kwadratów leżących w jednym 'pasku'
% zadanego rombu. Dalej przesuwamy pasek w prawo w dół.
for i = 1:n
    ...
    Sum = Sum + sum(partSum);
    vA_x = vA_x + d2; % Przesuwamy pasek kwadratów o w prawo w dół.
    vA_y = vA_y - d2;
    vB_x = vB_x + d2;
    vB_y = vB_y - d2;
    vC_x = vC_x + d2;
